...
LD_TASK void task_TEMP(void *pData)
{
    ...
    do {
        ...
        if (hal.DRV.auto_RESTART == PM_ENABLED && pm.lu_MODE == PM_LU_DISABLED && DRV_fault() != HAL_OK) {
            DRV_status();
        ...
        if (pm.fsm_errno != PM_OK) {
            ...
            if (tlm.watch_AUTO == PM_ENABLED && tlm.mode == TLM_MODE_DISABLED && pm.lu_MODE != PM_LU_DISABLED) {
...
#ifdef HW_HAVE_ANALOG_KNOB
static void conv_KNOB()
{
    float control, range, scaled;
    int hold_FLAG = PM_DISABLED;

    if (ap.knob_ACTIVE == PM_ENABLED && pm.lu_MODE == PM_LU_DISABLED) {
        ap.knob_ACTIVE = PM_DISABLED;
        ap.knob_DISARM = PM_ENABLED;
    }

    if (ap.knob_in_ANG < ap.knob_range_LOS[0] || ap.knob_in_ANG > ap.knob_range_LOS[1]) {
        scaled = -1.f;
...
#ifdef HW_HAVE_BRAKE_KNOB
    if (ap.knob_in_BRK < ap.knob_range_LOS[0] || ap.knob_in_BRK > ap.knob_range_LOS[1]) {
    ...
    if (m_fabsf(control) < M_EPSILON) {
        ...
        } else if (ap.knob_STARTUP == PM_ENABLED && ap.knob_DISARM == PM_ENABLED) {
            if (timeout_DISARM() == PM_ENABLED)
                ap.knob_DISARM = PM_DISABLED;
        }
    } else {
        if (ap.knob_STARTUP == PM_ENABLED && ap.knob_ACTIVE != PM_ENABLED) {
    ...
    if (ap.knob_reg_DATA != control || hold_FLAG == PM_ENABLED) {
...
static void default_flash_load()
{
...
static void conv_PULSE_WIDTH()
{
    float control, range, scaled;
    int hold_FLAG = PM_DISABLED;

    if (ap.ppm_ACTIVE == PM_ENABLED && pm.lu_MODE == PM_LU_DISABLED) {
    ...
    if (m_fabsf(control) < M_EPSILON) {
        ...
        } else if (ap.ppm_STARTUP == PM_ENABLED && ap.ppm_DISARM == PM_ENABLED) {
            if (timeout_DISARM() == PM_ENABLED)
                ap.ppm_DISARM = PM_DISABLED;
        }
    } else {
        if (ap.ppm_STARTUP == PM_ENABLED && ap.ppm_ACTIVE != PM_ENABLED) {
    ...
    if (ap.ppm_reg_DATA != control || hold_FLAG == PM_ENABLED) {
...
static void in_STEP_DIR()
{
    ...
    if (relEP != 0) {
        ...
        if (ap.step_STARTUP == PM_ENABLED) {
            if (pm.lu_MODE == PM_LU_DISABLED && pm.fsm_errno == PM_OK) {
                ap.step_POS = 0;
                pm.fsm_req = PM_STATE_LU_STARTUP;
            }
        }
...
void ADC_IRQ()
{
    ...
    } else if (hal.DPS_mode == DPS_DRIVE_ON_SPI && ap.proc_get_EP != NULL) {
    ...
    if (unlikely(hal.CNT_diag[1] >= pm.m_dT && pm.lu_MODE != PM_LU_DISABLED)) {
        pm.fsm_errno = PM_ERROR_HW_UNMANAGED_IRQ;
        pm.fsm_req = PM_STATE_HALT;
    }

#ifdef HW_HAVE_DRV_ON_PCB
    if (unlikely(DRV_fault() != HAL_OK && pm.lu_MODE != PM_LU_DISABLED)) {
...
#ifdef HW_HAVE_PWM_STOP
    if (unlikely(PWM_fault() != HAL_OK && pm.lu_MODE != PM_LU_DISABLED)) {
...
void app_control(const reg_t *reg, void (* pvTask) (void *), const char *pcName)
{
    ...
    if (reg->link->i == PM_ENABLED) {
        ...
        if (xHandle == NULL) {
            xTaskCreate(pvTask, pcName, configMINIMAL_STACK_SIZE, (void *)reg->link, AP_TASK_PRIORITY, NULL);
