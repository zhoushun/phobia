...
static int flash_prog_u8(flash_prog_t *pg, uint8_t u)
{
    int rc = 0;
...
static void flash_prog_u32(flash_prog_t *pg, uint32_t l)
{
    union {
        uint32_t l;
        uint8_t b[4];
...
static const char* flash_strncpy(char *d, const char *s, int len)
{
    do {
        if (--len < 0) {
            *d = 0;
...
static uint32_t flash_block_crc32(const flash_block_t *block)
{
    return crc32u(block, sizeof(flash_block_t) - sizeof(uint32_t));
}

static flash_block_t* flash_block_scan()
{
    ...
    do {
        if (flash_block_crc32(block) == block->crc32) {
            if (last != NULL) {
                last = (block->number > last->number) ? block : last;
            } else {
                last = block;
            }
        }
        ...
        if ((uint32_t)block >= FLASH_config.map[FLASH_config.total]) break;
    } while (1);
...
int flash_block_regs_load()
{
    ...
    while (*lsym != 0xFF) {
        ...
        if (*lsym == 0xBF) {
            ...
            if (reg != NULL && reg->mode & REG_CONFIG && reg->mode & REG_LINKED) {
...
static int flash_is_block_dirty(const flash_block_t *block)
{
...
static int flash_prog_config_regs(flash_block_t *block)
{
    const reg_t *reg;
    const char *lsym;
