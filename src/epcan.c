...
static void EPCAN_pipe_INCOMING(epcan_pipe_t *ep, const CAN_msg_t *msg)
{
    switch (ep->PAYLOAD) {
...
static void EPCAN_pipe_OUTGOING(epcan_pipe_t *ep)
{
    CAN_msg_t msg;
...
static void EPCAN_pipe_message_IN(const CAN_msg_t *msg)
{
    epcan_pipe_t *ep;
    int N;
    ...
    for (N = 0; N < EPCAN_EP_MAX; ++N) {
        ...
        if (ep->MODE == EPCAN_PIPE_INCOMING && EPCAN_ID_OFFSET(ep->ID) == msg->ID) {
            ep->tx_clock = 0;

            if (ep->ACTIVE == PM_ENABLED && pm.lu_MODE == PM_LU_DISABLED)
                ep->ACTIVE = PM_DISABLED;

            if (ep->STARTUP == PM_ENABLED && ep->ACTIVE != PM_ENABLED && pm.lu_MODE == PM_LU_DISABLED) {
                pm.fsm_req = PM_STATE_LU_STARTUP;
                ep->ACTIVE = PM_ENABLED;
            }

            EPCAN_pipe_INCOMING(ep, msg);
        } else if (ep->MODE == EPCAN_PIPE_OUTGOING_INJECTED && EPCAN_ID_OFFSET(ep->inject_ID) == msg->ID) {
            ep->tx_flag = 1;
...
void EPCAN_pipe_REGULAR()
{
    ...
    for (N = 0; N < EPCAN_EP_MAX; ++N) {
        ...
        if (ep->MODE == EPCAN_PIPE_INCOMING && ep->ACTIVE == PM_ENABLED) {
            ep->tx_clock++;
            ...
            if (ep->tx_clock >= ep->rate) {
                ...
                if (ep->STARTUP != PM_ENABLED || pm.lu_MODE != PM_LU_DISABLED)
                    ep->tx_flag = 1;
            }

            if (ep->tx_flag != 0)
                EPCAN_pipe_OUTGOING(ep);
        } else if (ep->MODE == EPCAN_PIPE_OUTGOING_INJECTED && ep->tx_flag != 0) {
            EPCAN_pipe_OUTGOING(ep);
        }
    }
}
...
void CAN_IRQ()
{
    ...
    if (hal.CAN_msg.ID >= EPCAN_ID_OFFSET(0U) && hal.CAN_msg.ID < EPCAN_ID_CAN(1U, 0U)) {
        EPCAN_pipe_message_IN(&hal.CAN_msg);
    }

    if (hal.CAN_msg.ID >= EPCAN_ID_CAN(1U, 0U) && hal.CAN_msg.ID < EPCAN_ID_CAN(32U, 0U)) {
...
static void local_node_discard()
{
    int N;
...
static void local_node_insert(uint32_t UID, int node_ID)
{
    int N;
    for (N = 0; N < EPCAN_NODE_MAX; ++N) {
...
static int local_node_valid_remote(int node_ID)
{
    if (node_ID == 0) return 0;

    int N;
    for (N = 0; N < EPCAN_NODE_MAX; ++N) {
        if (node_ID == local.node[N].node_ID && node_ID != net.node_ID) {
            return 1;
        }
    }
...
static int local_node_assign_ID()
{
    int node_ID, found_ID = 0;

    for (node_ID = 1; node_ID < EPCAN_NODE_MAX; ++node_ID) {
        if (local_node_valid_remote(node_ID) == 0 && node_ID != net.node_ID) {
            found_ID = node_ID;
            break;
        }
    }
...
static int local_node_random_ID()
{
    ...
    if (N > 0) found_ID = list_ID[urand() % N];
...
static void EPCAN_log_msg(CAN_msg_t *msg, const char *label)
{
    int N;
...
static void EPCAN_node_ACK_NET(int node_ACK)
{
    CAN_msg_t msg;
    int node_ID;
...
static void EPCAN_node_ACK_DATA(int node_ACK, int reg_ID)
{
    CAN_msg_t msg;
...
static void EPCAN_node_REQ_remote(int node_REQ)
{
    CAN_msg_t msg;
...
static void EPCAN_message_IN(const CAN_msg_t *msg)
{
    int N;

    /* Network functions.
     * */
    if (msg->ID == EPCAN_ID_NET_SURVEY && msg->len == 5U) {
        local_node_discard();
        local_node_insert(msg->payload.l[0], msg->payload.b[4]);

        EPCAN_node_ACK_NET(EPCAN_ACK_NETWORK_REPLY);
    } else if (msg->ID == EPCAN_ID_NET_ASSIGN && msg->len == 5U) {
     ...
     * */
        ...
        if (msg->payload.b[0] == EPCAN_REQ_FLOW_TX_PAUSE && msg->len == 1U) {
            local.flow_tx_paused = 1;
        } else if (msg->payload.b[0] == EPCAN_REQ_REG_GET && msg->len == 4U) {
            EPCAN_node_ACK_DATA(EPCAN_ACK_REG_DATA, msg->payload.s[1]);
        } else if (msg->payload.b[0] == EPCAN_REQ_REG_SET && msg->len == 8U) {
            reg_SET(msg->payload.s[1], (rval_t *) &msg->payload.l[1]);
        }
    } else if (EPCAN_GET_FUNC(msg->ID) == EPCAN_NODE_ACK) {
        if (msg->payload.b[0] == EPCAN_ACK_NETWORK_REPLY && msg->len == 8U) {
            local_node_insert(msg->payload.l[1], msg->payload.b[1]);
        } else if (msg->payload.b[0] == EPCAN_ACK_REG_DATA && msg->len == 8U) {
            if (msg->payload.b[1] == local.node_ACK_ID && local.node_ACK_ID != 0) {
...
SH_DEF(net_revoke)
{
    ...
    if (stoi(&node_ID, s) == NULL) return;

    for (N = 0; N < EPCAN_NODE_MAX; ++N) {
        if (local.node[N].UID != 0 && local.node[N].node_ID != 0) {
            if (local.node[N].node_ID == node_ID || node_ID == 0) {
                local.node[N].node_ID = 0;
...
SH_DEF(net_node_data)
{
    ...
    if (reg_ID != 0) {
        ...
        if (local.node_ACK_ID == 0 && local.reg_ID == reg_ID) {
