...
static void pub_font_layout(struct public *pub)
{
...
static int pub_primal_reg(struct public *pub, struct link_reg *reg)
{
...
static void pub_contextual(struct public *pub, struct link_reg *reg, struct nk_rect bounds)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;

    nk_style_push_vec2(ctx, &ctx->style.contextual_button.padding, nk_vec2(pub->fe_base * 1.5f, 4.0f));
    nk_style_push_vec2(ctx, &ctx->style.text.padding, nk_vec2(pub->fe_base * 1.5f, 4.0f));
    ...
    if (nk_contextual_begin(ctx, 0, nk_vec2(pub->fe_base * 26, 400), bounds)) {
        ...
        if (reg->mode & LINK_REG_CONFIG) {
            ...
            if (reg->mode & LINK_REG_TYPE_FLOAT) {
                sprintf(pub->lbuf, "# Range [%.22s; %.22s]", reg->vmin, reg->vmax);
...
static void pub_contextual_hidden(struct public *pub, const char *sym, struct nk_rect bounds)
{
    struct nk_sdl *nk = pub->nk;
...
static void pub_name_label(struct public *pub, const char *name, struct link_reg *reg)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
...
static void pub_name_label_hidden(struct public *pub, const char *name, const char *sym)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect bounds;
...
static int pub_combo_linked(struct public *pub, struct link_reg *reg,
        int item_height, struct nk_vec2 size)
{
    ...
    if (nk_combo_begin_label(ctx, lp->reg[select_ID].sym, size)) {
        ...
        nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD, pub->fe->fuzzy,
        ...
        for (reg_ID = 0; reg_ID < lp->reg_MAX_N; ++reg_ID) {
            if (lp->reg[reg_ID].sym[0] != 0) {
                if (strstr(lp->reg[reg_ID].sym, pub->fe->fuzzy) != NULL) {
                    if (nk_combo_item_label(ctx, lp->reg[reg_ID].sym, NK_TEXT_LEFT)) {
                        select_ID = reg_ID;
                    }
...
static struct nk_rect pub_get_popup_bounds_about(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
...
static struct nk_rect pub_get_popup_bounds_tiny(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect win, popup;
...
static struct nk_rect pub_get_popup_bounds_full(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect win, popup;
...
static void pub_popup_hwinfo(struct public *pub, int popup)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (pub->popup_enum != popup) return;

    bounds = pub_get_popup_bounds_about(pub);

    if (nk_popup_begin(ctx, NK_POPUP_DYNAMIC, " ", NK_WINDOW_CLOSABLE | NK_WINDOW_NO_SCROLLBAR, bounds)) {
        ...
        nk_edit_string_zero_terminated(ctx, NK_EDIT_SELECTABLE, lp->hw.revision, 79, nk_filter_default);
        nk_spacer(ctx);

        nk_spacer(ctx);
        nk_label(ctx, "Build date", NK_TEXT_LEFT);

        nk_edit_string_zero_terminated(ctx, NK_EDIT_SELECTABLE, lp->hw.build, 79, nk_filter_default);
        nk_spacer(ctx);

        nk_spacer(ctx);
        nk_label(ctx, "CRC32", NK_TEXT_LEFT);

        nk_edit_string_zero_terminated(ctx, NK_EDIT_SELECTABLE, lp->hw.crc32, 79, nk_filter_default);
        nk_spacer(ctx);
...
static void pub_popup_about(struct public *pub, int popup)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect bounds;
...
static void pub_popup_message(struct public *pub, int popup)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_popup_begin(ctx, NK_POPUP_DYNAMIC, " ", NK_WINDOW_CLOSABLE
        ...
        if (lp->command_state == LINK_COMMAND_RUNING || lp->command_state == LINK_COMMAND_WAITING) {
            nk_layout_row_template_begin(ctx, 0);
            nk_layout_row_template_push_variable(ctx, 1);
            nk_layout_row_template_push_static(ctx, pub->fe_base * 2);
            nk_layout_row_template_push_static(ctx, pub->fe_base * 2);
...
static void pub_popup_debug_log(struct public *pub, int popup, const char *title)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect bounds;
    ...
    if (nk_popup_begin(ctx, NK_POPUP_DYNAMIC, title, NK_WINDOW_CLOSABLE | NK_WINDOW_NO_SCROLLBAR, bounds)) {
        nk_layout_row_dynamic(ctx, pub->fe_font_h / 2, 1);
        nk_spacer(ctx);
        ...
        nk_spacer(ctx);
        nk_edit_string_zero_terminated(ctx, NK_EDIT_SELECTABLE | NK_EDIT_MULTILINE,
                pub->debug.log_message, sizeof(pub->debug.log_message), nk_filter_default);
        nk_spacer(ctx);
...
static void pub_popup_progress(struct public *pub, int popup, int pce)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect bounds;
    ...
    if (nk_popup_begin(ctx, NK_POPUP_DYNAMIC, "Reading", NK_WINDOW_TITLE | NK_WINDOW_NO_SCROLLBAR, bounds)) {
        nk_layout_row_dynamic(ctx, pub->fe_font_h / 2, 1);
        nk_spacer(ctx);
...
static int pub_popup_ok_cancel(struct public *pub, int popup, const char *msg)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
    struct nk_rect bounds;
    ...
    if (pub->popup_enum != popup) return 0;
...
static void pub_directory_scan(struct public *pub, const char *sup)
{
    int len, ofs, bsz, kmg, N;

    if (pub->fe->storage[0] != 0) {
        if (dirent_open(&pub->scan.sb, pub->fe->storage) != ENT_OK) return;
    } else {
        if (dirent_open(&pub->scan.sb, ".") != ENT_OK) return;
    }
    ...
    while (dirent_read(&pub->scan.sb) == ENT_OK) {
        if (pub->scan.sb.ntype == ENT_TYPE_REGULAR) {
            ...
            if (memcmp(pub->scan.sb.name + ofs, sup, len) == 0) {
                ...
                sprintf(pub->scan.file[N].size, "%4d %cb", (int)bsz, " KMG??" [kmg]);

                ++N;
                if (N >= PHOBIA_FILE_MAX - 1) break;
            }
        }
    }
...
static void pub_open_GP(struct public *pub, const char *file)
{
    if (pub->gp != NULL) gp_Clean(pub->gp);
    pub->gp = gp_Alloc();
...
static void reg_float_prog_um(struct public *pub, const char *sym, const char *name,
        float fmin, float fmax, int defsel);

static void pub_popup_telemetry_plot(struct public *pub, int popup)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_popup_begin(ctx, NK_POPUP_STATIC, " ", NK_WINDOW_CLOSABLE | NK_WINDOW_NO_SCROLLBAR, bounds)) {
        ...
        if (reg != NULL && reg->lval == 0) {
            if (nk_button_label(ctx, "Flush GP")) {
                config_storage_path(pub->fe, pub->lbuf, pub->telemetry.file_grab);

                strcpy(pub->telemetry.file_snap, pub->lbuf);

                if (link_grab_file_open(lp, pub->telemetry.file_snap) != 0) {
                    if (link_command(lp, "tlm_flush_sync") != 0) pub->telemetry.wait_GP = 1;
        ...
        if (reg != NULL && reg->lval == 0) {
            if (nk_button_label(ctx, "Stream GP")) {
                ...
                if (link_grab_file_open(lp, pub->telemetry.file_snap) != 0) {
                    if (link_command(lp, "tlm_stream_sync") != 0) pub->telemetry.wait_GP = 1;
                    reg = link_reg_lookup(lp, "tlm.mode");
                    if (reg != NULL) {
                        reg->lval = 3;
                        reg->onefetch = 1;
        ...
        nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD, pub->telemetry.file_grab,
                sizeof(pub->telemetry.file_grab), nk_filter_default);

        if (pub->scan.selected >= 0 && pub->scan.selected < PHOBIA_FILE_MAX) {
            N = pub->scan.selected;

            if (strcmp(pub->scan.file[N].name, pub->telemetry.file_grab) != 0) {
                pub->scan.selected = -1;
            }
        }
        ...
        if (nk_group_begin(ctx, "FILES", NK_WINDOW_BORDER)) {
            ...
            for (N = 0; N < PHOBIA_FILE_MAX; ++N) {
                if (pub->scan.file[N].name[0] == 0) break;

                sel = (N == pub->scan.selected) ? 1 : 0;

                newsel = nk_select_label(ctx, pub->scan.file[N].name, NK_TEXT_LEFT, sel);
                newsel |= nk_select_label(ctx, pub->scan.file[N].time, NK_TEXT_LEFT, sel);
                newsel |= nk_select_label(ctx, pub->scan.file[N].size, NK_TEXT_RIGHT, sel);

                if (newsel != sel && newsel != 0) {
                    pub->scan.selected = N;
                    strcpy(pub->telemetry.file_grab, pub->scan.file[N].name);
                }
            }
        ...
        if (pub->telemetry.wait_GP != 0 && lp->grab_N >= 5) {
        ...
        if (lp->grab_N != 0) link_grab_file_close(lp);
        pub->popup_enum = 0;
    }
}

static void pub_popup_config_export(struct public *pub, int popup)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_popup_begin(ctx, NK_POPUP_STATIC, " ", NK_WINDOW_CLOSABLE | NK_WINDOW_NO_SCROLLBAR, bounds)) {
        ...
        if (nk_button_label(ctx, "Export")) {
            config_storage_path(pub->fe, pub->lbuf, pub->config.file_grab);
        ...
        nk_spacer(ctx);
        nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD, pub->config.file_grab,
                sizeof(pub->config.file_grab), nk_filter_default);

        if (pub->scan.selected >= 0 && pub->scan.selected < PHOBIA_FILE_MAX) {
            N = pub->scan.selected;

            if (strcmp(pub->scan.file[N].name, pub->config.file_grab) != 0)
                pub->scan.selected = -1;
        }
        ...
        if (nk_group_begin(ctx, "FILES", NK_WINDOW_BORDER)) {
            ...
            for (N = 0; N < PHOBIA_FILE_MAX; ++N) {
                ...
                if (newsel != sel && newsel != 0) {
                    pub->scan.selected = N;
                    strcpy(pub->config.file_grab, pub->scan.file[N].name);
                }
            }
...
static void pub_drawing_machine_position(struct public *pub, float fpos[2], int dtype)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_widget(&space, ctx) == NK_WIDGET_INVALID) return;
    ...
    if (dtype == DRAWING_WITH_HALL) {
        ...
        for (N = 1; N <= 6; ++N) {
            ...
            if (reg == NULL) break;
            if ((reg->mode & LINK_REG_TYPE_FLOAT) == 0) break;
            ...
            col = (N == hall) ? nk->table[NK_COLOR_ENABLED] : nk->table[NK_COLOR_HIDDEN];
...
static void pub_drawing_flash_colored(struct nk_sdl *nk, const int sym)
{
    struct nk_context *ctx = &nk->ctx;
    struct nk_style_selectable select;
...
static void reg_enum_toggle(struct public *pub, const char *sym, const char *name)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL) {
        ...
        nk_edit_string_zero_terminated(ctx, NK_EDIT_SELECTABLE, pub->lbuf, 79, nk_filter_default);
...
static void reg_enum_errno(struct public *pub, const char *sym, const char *name, int onalert)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void reg_enum_get_item(void *userdata, int n, const char **item)
{
...
static void reg_enum_combo(struct public *pub, const char *sym, const char *name, int onlight)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL) {
        ...
        reg->lval = (reg->lval < reg->lmax_combo + 2 && reg->lval >= 0) ? reg->lval : 0;

        if (onlight != 0 && reg->lval != 0) {
        ...
        rc = nk_combo_callback(ctx, &reg_enum_get_item, reg, reg->lval, reg->lmax_combo + 2,
                pub->fe_font_h + 10, nk_vec2(pub->fe_base * 20, 400));
...
static void reg_text_large(struct public *pub, const char *sym, const char *name)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL) {
        ...
        pub_name_label(pub, name, reg);
        nk_edit_string_zero_terminated(ctx, NK_EDIT_SELECTABLE, reg->val, 79, nk_filter_default);
        nk_spacer(ctx);
        nk_spacer(ctx);
        nk_spacer(ctx);
...
static void reg_float(struct public *pub, const char *sym, const char *name)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL) {
        ...
        if (reg->mode & LINK_REG_READ_ONLY) {
            ...
            int rc = nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD | NK_EDIT_SIG_ENTER,
...
static void reg_um_get_item(void *userdata, int n, const char **item)
{
    struct link_reg *reg = (struct link_reg *)userdata;
    *item = reg[n].um;
}

static void reg_float_um(struct public *pub, const char *sym, const char *name, int range, int defsel)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (rc != 0 && max_ID >= min_ID) {
        ...
        if (reg->mode & LINK_REG_READ_ONLY) {
            ...
            rc = nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD | NK_EDIT_SIG_ENTER, reg->val,
                    79, nk_filter_default);
            if (rc & (NK_EDIT_DEACTIVATED | NK_EDIT_COMMITED))
                reg->modified = lp->clock;
        }
        ...
        if (max_ID != min_ID) {
            rc = nk_combo_callback(ctx, &reg_um_get_item, &lp->reg[min_ID], lp->reg[min_ID].um_sel,
                    (max_ID - min_ID) + 1, pub->fe_font_h + 10, nk_vec2(pub->fe_base * 8, 400));
...
static void reg_linked(struct public *pub, const char *sym, const char *name)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && (reg->mode & LINK_REG_LINKED) != 0) {
        pub_name_label(pub, name, reg);
        rc = pub_combo_linked(pub, reg, pub->fe_font_h + 5, nk_vec2(pub->fe_base * 20, 400));
        if (rc != reg->lval) {
            sprintf(reg->val, "%i", rc);
            reg->modified = lp->clock;
        }
...
static void reg_float_prog_by_ID(struct public *pub, int reg_ID)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && (reg->mode & LINK_REG_CONFIG) == 0) {
        if (reg->fetched + 500 > lp->clock) {
            struct nk_color flash;
            flash = nk->table[NK_COLOR_FLICKER_LIGHT];
        ...
        if (reg->started != 0 && reg->fmin < reg->fmax) {
            pce = (int)(1000.f * (reg->fval - reg->fmin) / (reg->fmax - reg->fmin));
        }
        ...
        float fmin, float fmax, int defsel)
{
    ...
    if (rc != 0 && max >= min) {
        ...
        if (fmin < fmax) {
            /* Use external range */
        } else if (reg->started != 0 && reg->fmin < reg->fmax) {
            fmin = reg->fmin;
            fmax = reg->fmax;
        ...
        if (reg->mode & LINK_REG_READ_ONLY) {
            ...
            nk_spacer(ctx);
            pub_contextual(pub, reg, bounds);
            rc = nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD | NK_EDIT_SIG_ENTER,
                    reg->val, 79, nk_filter_default);
        ...
        if (max != min) {
            rc = nk_combo_callback(ctx, &reg_um_get_item, &lp->reg[min], lp->reg[min].um_sel,
                    (max - min) + 1, pub->fe_font_h + 10, nk_vec2(pub->fe_base * 8, 400));
...
static void page_serial(struct public *pub)
{
    struct config_phobia *fe = pub->fe;
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    ...
    if (lp->linked == 0) {
        ...
        pub->serial.selected = nk_combo(ctx, pub->serial.list.name, pub->serial.list.dnum,
                pub->serial.selected, pub->fe_font_h + 10, nk_vec2(pub->fe_base * 13, 400));

        nk_spacer(ctx);
        if (nk_button_label(ctx, "Scan")) serial_enumerate(&pub->serial.list);
        nk_spacer(ctx);

        nk_label(ctx, "Baudrate", NK_TEXT_LEFT);
        pub->serial.baudrate = nk_combo(ctx, ls_baudrate, 4, pub->serial.baudrate,
        ...
        if (nk_button_label_styled(ctx, &orange, "Connect")) {
            ...
            mode = (pub->serial.parity == 1) ? "8E1" : (pub->serial.parity == 2) ? "8O1" : "8N1";
        ...
        pub->serial.parity = nk_combo(ctx, ls_parity, 3, pub->serial.parity,
                pub->fe_font_h + 10, nk_vec2(pub->fe_base * 13, 400));
        ...
        if (nk_button_label_styled(ctx, &orange, "Drop")) {
            ...
            if (lp->reg_MAX_N > 300 && lp->reg_MAX_N < LINK_REGS_MAX) {
    ...
    fe->windowsize = (fe->windowsize < 0) ? 0 : (fe->windowsize > 2) ? 2 : fe->windowsize;
    ...
    rc = nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD | NK_EDIT_SIG_ENTER, fe->storage,
    ...
    rc = nk_edit_string_zero_terminated(ctx, NK_EDIT_FIELD | NK_EDIT_SIG_ENTER, fe->fuzzy,
    ...
    nk_label_colored(ctx, fe->rcfile, NK_TEXT_LEFT, nk->table[NK_COLOR_HIDDEN]);
    nk_spacer(ctx);
    ...
    if (pub_popup_ok_cancel(pub, POPUP_RESET_DEFAULT, "Please confirm that you really"
                " want to reset all local Phobia frontend configuration.") != 0) {
...
static void page_diagnose(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_menu_begin_label(ctx, "Self-Test", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
    ...
    if (nk_menu_begin_label(ctx, "Advanced", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
    ...
    if (nk_menu_begin_label(ctx, "Debug", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
    ...
    if (pub->debug.log_flush != 0 && lp->grab_N == 0) {

        pub->debug.log_flush = 0;

        if (lp->linked != 0) {
            FILE *fd = fopen_from_UTF8(pub->debug.file_snap, "rb");
            if (fd != NULL) {
                int len = fread(pub->debug.log_message, 1, sizeof(pub->debug.log_message) - 1, fd);
                if (len != 0) {
    ...
    if (reg != NULL && reg->fval == 1.0f) {
        ...
        nk_label_colored(ctx,  "NOTE: Do the self-test and self-adjustment from `Test` menu", NK_TEXT_LEFT, warning);
        nk_layout_row_dynamic(ctx, 0, 1);
        nk_spacer(ctx);
    }
    ...
    if (pub_popup_ok_cancel(pub, POPUP_RESET_DEFAULT, "Please confirm that you really"
                " want to reset all adjustment constants related to the board.") != 0) {
        if (link_command(lp, "pm_default_scale") != 0) {
            link_reg_fetch_all_shown(lp);
        }
    }

    if (lp->unable_warning != 0 && pub->popup_enum == POPUP_NONE) {
        pub->popup_enum = POPUP_UNABLE_WARNING;
        lp->unable_warning = 0;
    }

    if (lp->command_state == LINK_COMMAND_RUNING && lp->locked < lp->clock + 100
...
static void page_probe(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_menu_begin_label(ctx, "Probe", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
    ...
    if (nk_menu_begin_label(ctx, "Config", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
    ...
    if (pub->debug.log_GP != 0 && lp->grab_N >= 5) {
        pub->debug.log_GP = 0;
    ...
    if (reg != NULL && reg->modified == lp->clock) {
    ...
    reg = link_reg_lookup(lp, "pm.const_Zp");
    if (reg != NULL && reg->fetched == lp->clock) {
    ...
    if (config_LU_DRIVE == LU_DRIVE_CURRENT || config_LU_DRIVE == LU_DRIVE_TORQUE
            || config_LU_DRIVE == LU_DRIVE_SPEED) {
        reg = link_reg_lookup(lp, "pm.lu_MODE");
        if (reg != NULL) {
            reg->update = 1000;

            int rate = (reg->lval != 0) ? 100 : 0;
    ...
    if (pub_popup_ok_cancel(pub, POPUP_PROBE_DETACHED, "PMC will wait for the machine to reach"
                " some speed. You will have to rotate the machine rotor manually.") != 0) {
    ...
    if (pub_popup_ok_cancel(pub, POPUP_RESET_DEFAULT, "Please confirm that you really"
                " want to reset all measured constants related to the machine.") != 0) {
...
static void page_hal(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct nk_context *ctx = &nk->ctx;
...
static void page_in_network(struct public *pub)
{
    struct config_phobia *fe = pub->fe;
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    ...
    if (N >= 0 && N < LINK_EPCAN_MAX && lp->epcan[N].UID[0] != 0) {
        ...
        if (strstr(lp->network, "REMOTE") == NULL) {
            if (nk_button_label_styled(ctx, &orange, "Connect")) {
                sprintf(pub->lbuf, "net_node_remote %.7s", lp->epcan[N].node_ID);
    ...
    if (nk_group_begin(ctx, "NETWORK", NK_WINDOW_BORDER)) {
        ...
        for (N = 0; N < LINK_EPCAN_MAX; ++N) {
            if (lp->epcan[N].UID[0] == 0) break;

            sel = (N == pub->network.selected) ? 1 : 0;

            newsel = nk_select_label(ctx, lp->epcan[N].UID, NK_TEXT_LEFT, sel);
            newsel |= nk_select_label(ctx, lp->epcan[N].node_ID, NK_TEXT_LEFT, sel);

            if (newsel != sel && newsel != 0) pub->network.selected = N;
        }
    ...
    if (pub->popup_enum == POPUP_LINK_PROGRESS) {
        int link_pce = (int) (1000.f * lp->reg_MAX_N / (float)fe->regfile);
...
static void page_in_stepdir(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval == 0) {
        ...
        nk_label_colored(ctx,  "NOTE: Select STEP mode `STEP_ON_STEP_DIR` on `HAL` page",
...
static void page_in_pwm(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval != 1) {
        ...
        nk_label_colored(ctx,  "NOTE: Select PPM mode `PPM_PULSE_WIDTH` on `HAL` page",
                NK_TEXT_LEFT, warning);
...
static void page_in_knob(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_menu_begin_label(ctx, "Knob", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
...
static void page_application(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval != 0) {
        ...
        if (reg != NULL && reg->lval != 3) {
            ...
            nk_label_colored(ctx,  "NOTE: Select DPS mode `DPS_DRIVE_ON_SPI` on `HAL` page",
                    NK_TEXT_LEFT, warning);
            nk_layout_row_dynamic(ctx, 0, 1);
            nk_spacer(ctx);
        }
...
static void page_thermal(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_config(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_menu_begin_label(ctx, "Menu", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
...
static void page_lu_forced(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_lu_flux(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_lu_hfi(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval != 2) {
        ...
        nk_label_colored(ctx,  "NOTE: Select sensorless estimate `PM_FLUX_KALMAN` on `Config` page",
                    NK_TEXT_LEFT, warning);
        nk_layout_row_dynamic(ctx, 0, 1);
        nk_spacer(ctx);
    }
...
static void page_lu_hall(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval != 1) {
        ...
        nk_label_colored(ctx,  "NOTE: Select DPS mode `DPS_DRIVE_HALL` on `HAL` page",
                NK_TEXT_LEFT, warning);
        nk_layout_row_dynamic(ctx, 0, 1);
        nk_spacer(ctx);
    }
...
static void page_lu_eabi(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval != 2) {
        ...
        nk_label_colored(ctx,  "NOTE: Select DPS mode `DPS_DRIVE_EABI` on `HAL` page",
                NK_TEXT_LEFT, warning);
...
static void page_lu_sincos(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_wattage(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_lp_current(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_lp_speed(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void page_lp_location(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (reg != NULL && reg->lval != LU_DRIVE_LOCATION) {
        ...
        nk_label_colored(ctx,  "NOTE: Select DRIVE control loop `PM_DRIVE_LOCATION` on `Config` page",
                NK_TEXT_LEFT, warning);
        nk_layout_row_dynamic(ctx, 0, 1);
        nk_spacer(ctx);
    }
    ...
    if (reg != NULL && (reg + reg->um_sel)->fetched == lp->clock) {
    ...
    reg_float_prog_um(pub, "pm.lu_location", "LU location", ld_range[0], ld_range[1], um_def);
...
static void page_telemetry(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_menu_begin_label(ctx, "Grab", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
    ...
    if (nk_menu_begin_label(ctx, "Analyse", NK_TEXT_CENTERED, nk_vec2(pub->fe_base * 16, 800))) {
        nk_layout_row_dynamic(ctx, 0, 1);
...
static void page_flash(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (pub_popup_ok_cancel(pub, POPUP_FLASH_WIPE, "Please confirm that you really want to WIPE flash storage.") != 0) {
        link_command(lp, "flash_wipe\r\nflash_info");
    }

    if (pub_popup_ok_cancel(pub, POPUP_SYSTEM_REBOOT, "Please confirm that you really want to reboot PMC.") != 0) {
    ...
    if (nk_group_begin(ctx, "INFO", NK_WINDOW_BORDER)) {
        ...
        for (bN = 0; bN < LINK_FLASH_MAX; ++bN) {
            if (lp->flash[0].block[bN] == 0) break;
...
static void page_upgrade(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (pub_popup_ok_cancel(pub, POPUP_SYSTEM_BOOTLOAD, "Please confirm that you really want to"
                " reboot into embedded bootloader. Note that PMC connection will be closed.") != 0) {
...
static void menu_select_button(struct public *pub, const char *title, void (* pfunc) (struct public *))
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
...
static void menu_group_layout(struct public *pub)
{
    struct nk_sdl *nk = pub->nk;
    struct link_pmc *lp = pub->lp;
    struct nk_context *ctx = &nk->ctx;
    ...
    if (nk_group_begin(ctx, "PAGE", 0)) {
        ...
        if (pub_popup_ok_cancel(pub, POPUP_LINK_WARNING, "A sudden reset was detected so we must drop"
                    " this link and connect to PMC again to read actual information.") != 0) {
            config_write(pub->fe);
            link_close(lp);
        }
...
int main(int argc, char **argv)
{
    ...
    nk->surface = SDL_CreateRGBSurfaceWithFormat(0, nk->fb->w, nk->fb->h, 32, SDL_PIXELFORMAT_XRGB8888);
    if (nk->surface == NULL) {
        /* TODO */
    }
    ...
    while (nk->onquit == 0) {
        ...
        while (SDL_PollEvent(&ev) != 0) {
            if (ev.window.windowID == nk->window_ID || ev.type == SDL_QUIT) {
                nk_sdl_input_event(nk, &ev);
            } else if (pub->gp != NULL && ev.window.windowID == pub->gp_ID) {
        ...
        if (nk->active != 0) {
            struct nk_rect bounds = nk_rect(0, 0, nk->surface->w, nk->surface->h);
            ...
            if (lp->hwinfo[0] != 0) {
                ...
                if (lp->network[0] != 0) {
                    sprintf(pub->lbuf + strlen(pub->lbuf), " / %.16s", lp->network);
                }

                nk->idled = 0;
            } else {
                nk->ctx.style.window.header.active = nk_style_item_color(nk->table[NK_COLOR_HEADER]);
                sprintf(pub->lbuf, " Not linked to PMC");
            }

            if (nk_begin(&nk->ctx, pub->lbuf, bounds, NK_WINDOW_TITLE | NK_WINDOW_NO_SCROLLBAR)) {
        ...
        if (pub->gp != NULL && gp_IsQuit(pub->gp) == 0) {
            if (gp_Draw(pub->gp) == 0) {
                SDL_Delay(10);
            }
        } else {
