...
#ifdef _WINDOWS
static void config_ACP_to_UTF8(char *lputf, const char *lpacp, int len)
{
...
static int config_rcfiletry(struct config_phobia *fe)
{
    FILE *fd;
    int rc = 0;
...
void config_read(struct config_phobia *fe)
{
    ...
    if (fd != NULL) {
        while (fgets(line, sizeof(line), fd) != NULL) {
            ...
            if (name == NULL) goto config_read_SKIP;
            if (value == NULL) value = "";
