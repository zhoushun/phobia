...
{
    int len = dw->pixmap.yspan * dw->pixmap.h;
...
static int clipCode(clipBox_t *cb, double x, double y)
{
    int cc;

    if (x < cb->min_x) cc = 0x01UL;
    else if (x > cb->max_x) cc = 0x02UL;

    if (y < cb->min_y) cc |= 0x04UL;
    else if (y > cb->max_y) cc |= 0x08UL;
...
static int clipLine(clipBox_t *cb, double *xs, double *ys, double *xe, double *ye)
{
    double dx, dy;
    int s_cc, e_cc;
...
static void drawRoughLine(SDL_Surface *surface, int xs, int ys, int xe, int ye, Uint32 col)
{
    Uint32 *pixels = (Uint32 *)surface->pixels;
...
static void drawRoughDash(draw_t *dw, SDL_Surface *surface, int xs, int ys, int xe, int ye,
    ...
    if (g != NULL) {
        ...
        if (clipLine(cb, &_fxs, &_fys, &_fxe, &_fye) == 0) {
            svgDrawLine(g, _fxs, _fys, _fxe, _fye,
                    (svgCol_t)dw->palette[ncol], thickness, 0, 0);
        }
    }
        ...
        double fxe, double fye, int ncol, int thickness, int dash, int space)
{
    ...
    if (g != NULL) {
        ...
        if (clipLine(cb, &_fxs, &_fys, &_fxe, &_fye) == 0) {
            svgDrawLine(g, _fxs, _fys, _fxe, _fye, (svgCol_t)dw->palette[ncol],
        ...
        double fxe, double fye, int ncol, int thickness)
{
    ...
    if (ncol == dw->cached_ncol && xs == xe && xe == dw->cached_x) {
        if (ys < ye) {
        ...
        int xe, int ye, Uint32 col)
{
    ...
    if (xs > cb->max_x || xe < cb->min_x) return;
    if (ys > cb->max_y || ye < cb->min_y) return;
        ...
        int rsize, int ncol, int round)
{
    ...
    if (fxs > cb->max_x + 16 || fxs < cb->min_x - 16) return;
    if (fys > cb->max_y + 16 || fys < cb->min_y - 16) return;

    if (g != NULL) {
        if (fxs > cb->min_x && fxs < cb->max_x && fys > cb->min_y && fys < cb->max_y) {
            if (round == 0) {
                svgDrawRect(g, fxs - rsize * 0.5, fys - rsize * 0.5,
                        fxs + rsize * 0.5, fys + rsize * 0.5, (svgCol_t)dw->palette[ncol]);
            } else {
                svgDrawCircle(g, fxs, fys, rsize * 0.5, (svgCol_t)dw->palette[ncol]);
            }
        }
    }
        ...
        int rsize, int ncol, int round)
{
    ...
    if (fxs > cb->max_x + 16 || fxs < cb->min_x - 16) return 0;
