...
#endif
{
    ...
    if (unlikely( rm->keep >= ls->n_threshold && ls->n_cascades >= 2)) {
        if (rm < LSE_RM_TOP(ls)) {
...
static void lse_qrmerge(lse_t *ls, lse_upper_t *rm, lse_upper_t *um)
{
    lse_float_t *m = um->m;
#if LSE_FAST_GIVENS != 0
    lse_float_t *d = um->d;
    ...
    n = (um->lazy != 0) ? um->rows : (um->rows < um->keep) ? um->rows : um->keep;
...
static void lse_qrfinal(lse_t *ls)
{
...
static void lse_qrstep(lse_t *ls, lse_upper_t *um, lse_upper_t *im, lse_float_t *u)
{
    lse_float_t *mq = im->m;
#if LSE_FAST_GIVENS != 0
    lse_float_t *d = um->d;
...
void lse_forget(lse_t *ls, lse_float_t la)
{
    ...
    for (i = 0; i < ls->n_cascades; ++i) {
        ...
        n = (rm->lazy != 0) ? rm->rows : (rm->rows < rm->keep) ? rm->rows : rm->keep;
