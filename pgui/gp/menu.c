...
static int menuFuzzy(menu_t *mu, const char *list, const char *fu, int light[])
{
...
static void menuBuild(menu_t *mu, int rep)
{
...
static int menuItemHover(menu_t *mu)
{
...
static int menuConvFuzzy(menu_t *mu, int fuzzy_N)
{
    const char *list = mu->list;
    int N = 0, K = 0, rN = -1;
...
static void menuScrollClick(menu_t *mu)
{
        ...
        if (menuFuzzy(mu, list, mu->fuzzy, light)) {
            ...
            if (mu->colorful != 0 && N_conv != -1) {
                ...
                baseX = (mu->scroll_limit != 0)
                    ? mu->size_X - mu->layout_height : mu->size_X;
