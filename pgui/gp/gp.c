...
static int gpGetBlankData(gpcon_t *gp)
{
    plot_t *pl = gp->pl;
    read_t *rd = gp->rd;
    ...
    for (N = 0; N < PLOT_DATASET_MAX; ++N) {
        ...
        if ( rd->data[N].format != FORMAT_NONE && rd->data[N].column_N != 0) {
...
static void gpMakeBlank(gpcon_t *gp, int length)
{
    read_t *rd = gp->rd;
    int N, dN;
...
static void gpFileGetPath(gpcon_t *gp)
{
    char *home;
...
static void gpFileGetLocal(gpcon_t *gp)
{
#ifdef _WINDOWS
...
static void gpDefaultFile(gpcon_t *gp)
{
    FILE *fd;
...
static void gpWriteFile(gpcon_t *gp)
{
    draw_t *dw = gp->dw;
    plot_t *pl = gp->pl;
    read_t *rd = gp->rd;
...
static int gpFileExist(const char *file)
{
    unsigned long long nsize;
    return (file_stat(file, &nsize) == ENT_OK) ? 1 : 0;
}

static int gpFullLength(plot_t *pl)
{
    return pl->screen.max_x / pl->layout_font_long;
}

static void gpTextLeftCrop(char *sbuf, const char *text, int allowed)
{
    int length;
...
static void gpTextFull(plot_t *pl, char *sbuf, const char *text, int margin)
{
    gpTextLeftCrop(sbuf, text, gpFullLength(pl) - margin);
}

static void gpTextSpaceAdd(char *sbuf, int len)
{
    sbuf += strlen(sbuf);
    memset(sbuf, ' ', len);
    sbuf[len] = 0;
...
static void gpTextSepFull(char *sbuf, int len)
{
    memset(sbuf, '-', len);
    sbuf[len] = 0;
}

static void gpTextFloat(plot_t *pl, char *sbuf, double val)
{
...
static void gpScreenLayout(gpcon_t *gp)
{
    plot_t *pl = gp->pl;
    menu_t *mu = gp->mu;
    edit_t *ed = gp->ed;
...
static void gpFontLayout(gpcon_t *gp)
{
    plot_t *pl = gp->pl;
    menu_t *mu = gp->mu;
    edit_t *ed = gp->ed;
...
static void gpFontHinting(gpcon_t *gp)
{
    plot_t *pl = gp->pl;
...
static void gpFontToggle(gpcon_t *gp, int toggle, int font_pt)
{
    plot_t *pl = gp->pl;
    int style;
...
static void gpMakePageMenu(gpcon_t *gp)
{
    read_t *rd = gp->rd;
    char *la = gp->la_menu;
    int pN = 0;
    ...
    do {
        if (rd->page[pN].busy != 0) {
            gpTextFull(gp->pl, gp->sbuf[1], rd->page[pN].title, gp->layout_menu_page_margin);
        ...
        if (pN >= READ_PAGE_MAX) break;
    } while (1);

    *la = 0;
}

static int gpFileIsGP(const char *file)
{
    int rc = 0;
...
#ifdef _LEGACY
static int legacy_FileIsBAT(const char *file)
{
    int rc = 0;
...
static char *legacy_DirName(char *path)
{
    char *eol;
...
static void legacy_FileOpenBAT(gpcon_t *gp, const char *file, int fromUI)
{
    FILE *fd;
    char *s, *path, *argv[2];
    int line_N = 0;
    ...
    do {
        ...
        if (s != NULL && (    strstr(s, "grm") != NULL || strstr(s, "GRM") != NULL)) {
...
static void gpUnifiedFileOpen(gpcon_t *gp, const char *file, int fromUI)
{
    read_t *rd = gp->rd;
...
static int gpDirWalk(gpcon_t *gp, int dir_N, int revert);

static void gpMakeDirMenu(gpcon_t *gp)
{
    char sfmt[PLOT_STRING_MAX];
    char *la = gp->la_menu;
    int len, pad, N, kmg;
    ...
    if (gp->cwd_exist != 0) {
        while (dirent_read(&gp->sb) == ENT_OK) {
            if (gp->sb.ntype == ENT_TYPE_DIRECTORY) {
                if ( strcmp(gp->sb.name, ".") == 0 || strcmp(gp->sb.name, "..") == 0) {
                    continue;
                }

                if (N >= GP_FILE_ENT_MAX) break;
                ...
                if (strlen(gp->sbuf[0]) < PLOT_STRING_MAX) {
                    ...
                    gpTextFull(gp->pl, gp->sbuf[1], gp->sb.name, gp->layout_menu_dir_margin);

                    pad = len - utf8_length(gp->sbuf[1]);

                    sprintf(sfmt, "/%%s%%%ds %%16s", pad);
                    sprintf(gp->sbuf[0], sfmt, gp->sbuf[1], "", gp->sb.time);
            ...
            if (gp->sb.ntype == ENT_TYPE_REGULAR) {
                ...
                if (strlen(gp->sbuf[0]) < PLOT_STRING_MAX) {
                    ...
                    sprintf(gp->sbuf[0], sfmt, gp->sbuf[1], "",
                            gp->sb.time, (int)gp->sb.nsize, " KMG??" [kmg]);
                    strcpy(la, gp->sbuf[0]);
                    la += strlen(la) + 1;
...
static int gpDirWalk(gpcon_t *gp, int dir_N, int revert)
{
    ...
    if (dir_N == 0) {
        editRaise(gp->ed, 9, gp->la->file_name_edit, gp->cwd, gp->mu->box_X, gp->mu->box_Y);
        gp->stat = GP_EDIT;
    } else if (dir_N == 1) {
        walk = 2;
    } else if (dir_N == 2) {
...
static const char * gpSymDatasetFormat(gpcon_t *gp, int dN)
{
    read_t *rd = gp->rd;
    const char *sym;
...
static void gpMenuInsertColumn(gpcon_t *gp, int dN, int cN)
{
    read_t *rd = gp->rd;
...
static void gpMenuInsertDataset(gpcon_t *gp, int dN)
{
    read_t *rd = gp->rd;
...
static void gpMakeColumnSelectMenu(gpcon_t *gp, int dN)
{
    read_t *rd = gp->rd;
    char *la = gp->la_menu;
    int sN, cN = 0;
...
static void gpMakeDatasetSelectMenu(gpcon_t *gp)
{
    char *la = gp->la_menu;
    int dN = 0, len, fspace;
...
static void gpMakeDatasetMenu(gpcon_t *gp)
{
    plot_t *pl = gp->pl;
    read_t *rd = gp->rd;
    char *la = gp->la_menu;
...
static void gpMakeConfigurationMenu(gpcon_t *gp)
{
    read_t *rd = gp->rd;
...
static void gpWriteNewConfiguration(gpcon_t *gp)
{
    FILE *fd;
    int line_N;
...
static void gpMakeResampleMenu(gpcon_t *gp)
{
    plot_t *pl = gp->pl;
    read_t *rd = gp->rd;
    char *la = gp->la_menu;
...
static void gpMakeAboutMenu(gpcon_t *gp)
{
    char *la = gp->la_menu;
...
static void gpTakeScreen(gpcon_t *gp)
{
...
#ifdef _WINDOWS
static void legacy_SetClipboard(SDL_Surface *surface)
{
    long length;
    void *mBMP;
    SDL_RWops *rwops;
...
static void gpYankScreen(gpcon_t *gp)
{
    if (gp->screen_yank != 0) {
...
static void gpMenuHandle(gpcon_t *gp, int menu_N, int item_N)
{
    draw_t *dw = gp->dw;
    plot_t *pl = gp->pl;
    read_t *rd = gp->rd;
    ...
    if (menu_N == 1) {
        ...
        if (item_N != -1) {
            if ( item_N < PLOT_DATASET_MAX && item_N >= 0) {
                gp->data_N = item_N;
            }
        ...
        case 0:
            ...
            if (pl->axis[gp->ax_N].slave != 0) {
                ...
                mu->hidden_N[1] = 1;
                mu->hidden_N[2] = 2;
            } else if ( gp->ax_N == pl->on_X || gp->ax_N == pl->on_Y) {
                mu->hidden_N[0] = 2;
            }
...
static void gpEditHandle(gpcon_t *gp, int edit_N, const char *text)
{
    scheme_t *sch = gp->sch;
    lang_t *la = gp->la;
    draw_t *dw = gp->dw;
...
static void gpEventHandle(gpcon_t *gp, const SDL_Event *ev)
{
    plot_t *pl = gp->pl;
    read_t *rd = gp->rd;
    menu_t *mu = gp->mu;
    ...
    if (gp->stat == GP_IDLE) {
        if (ev->type == SDL_KEYDOWN) {
            if (ev->key.keysym.sym == SDLK_a) {
                ...
                if (pl->hover_axis != -1) {
                    if ( pl->hover_axis != pl->on_X && pl->hover_axis != pl->on_Y) {
                        mu->box_X = gp->cur_X;
                        mu->box_Y = gp->cur_Y;
            ...
            if (ev->button.button == SDL_BUTTON_LEFT) {
                ...
                do {
                    ...
                    if (N >= 0) {
                        if ( pl->axis[N].busy == AXIS_BUSY_X && pl->axis[N].slave == 0) {
                            pl->on_X = N;
                            gp->ax_N = N;

                            gp->stat = GP_MOVING;
                        } else if ( pl->axis[N].busy == AXIS_BUSY_Y && pl->axis[N].slave == 0) {
                            pl->on_Y = N;
                            gp->ax_N = N;
                            gp->stat = GP_MOVING;
                        }
                ...
                do {
                    if (gp->cur_Y < gp->layout_page_box) {
                        menuRaise(mu, 1, gp->la->global_menu, gp->cur_X, gp->cur_Y);
                    ...
                    if (N >= 0) {
                        gp->fig_N = N;
                        menuRaise(mu, 3, gp->la->figure_menu, gp->cur_X, gp->cur_Y);
                        gp->stat = GP_MENU;
                        break;
                    }
                    ...
                    if (N == 0) {
                        menuRaise(mu, 4, gp->la->legend_menu, gp->cur_X, gp->cur_Y);
                        mu->mark[0].N = 5;
                        mu->mark[0].subs = (pl->transparency == 0) ? " " : "X";
                        mu->mark[1].N = 6;
                        mu->mark[1].subs = (pl->legend_hidden == 0) ? " " : "X";
                    ...
                    if (N == 0) {
                        menuRaise(mu, 5, gp->la->databox_menu, gp->cur_X, gp->cur_Y);
                        sprintf(gp->sbuf[2], "%1i", pl->fprecision);
                        mu->mark[0].N = 1;
                        mu->mark[0].subs = gp->sbuf[2];
                        mu->mark[1].N = 2;
                    ...
                    if (N >= 0) {
                        if (N != pl->on_X && N != pl->on_Y) {
                            gp->ax_N = N;
                            menuRaise(mu, 2, gp->la->axis_menu, gp->cur_X, gp->cur_Y);
                            mu->mark[0].N = 4;
            ...
            if (gp->shift_on != 0) {
                fmin = (ev->wheel.y > 0) ? 1.01 : (ev->wheel.y < 0) ? 1. / 1.01 : 1.;
            } else {
                fmin = (ev->wheel.y > 0) ? 1.1 : (ev->wheel.y < 0) ? 1. / 1.1 : 1.;
            }
...
static void gpDrawRangeLight(SDL_Surface *surface, gpcon_t *gp)
{
    plot_t *pl = gp->pl;
...
static void gpDrawRangeSelect(SDL_Surface *surface, gpcon_t *gp)
{
    draw_t *dw = gp->dw;
    plot_t *pl = gp->pl;
...
static void gpDrawBoxLight(SDL_Surface *surface, gpcon_t *gp)
{
    plot_t *pl = gp->pl;
...
static void gpDrawBoxSelect(SDL_Surface *surface, gpcon_t *gp)
{
    draw_t *dw = gp->dw;
    plot_t *pl = gp->pl;
...
static void gpFPSUpdate(gpcon_t *gp)
{
    gp->fps_frame += 1;
...
int gp_Draw(gpcon_t *gp)
{
    ...
    if (gp->unfinished != 0) {
        ...
        if ( rd->fastdraw != 0 && dw->antialiasing != DRAW_SOLID) {
            t0 = SDL_GetTicks();
        }
        ...
        if ( gp->window != NULL && gp->fps_show != 0) {
            int len, jam;
...
static void gpGetCMD(gpcon_t *gp, int argn, char *argv[])
{
    read_t *rd = gp->rd;
    char *op;
    int failed, argi;
...
static void gpHelloPage(gpcon_t *gp)
{
...
#ifdef _WINDOWS
static void legacy_CloseConsole()
{
    DWORD pLIST[4];
