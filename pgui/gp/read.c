...
static void readCutLabel(char *tbuf, const char *text, int allowed)
{
    int length;
...
static void readCutFile(read_t *rd, char *tbuf, const char *file, int allowed)
{
    const char *eol;
    int length, ndir;
...
static FILE* legacy_fopen_from_UTF8(const char *file, const char *mode)
{
...
#ifdef _LEGACY
static int legacy_LabelExtract(char *s, char *label[9])
{
    char *cur;
    int m, N;
...
static char* legacy_TextTrim(read_t *rd, char *s)
{
    char *eol;
        ...
        const char *file, int fromUI)
{
    ...
    if (fd == NULL) {
        ...
        if (len != 0 && fpN == (float)cN && cN > 1 && cN < READ_COLUMN_MAX) {
            fclose(fd);
    ...
    if (fd == NULL) {
        ...
        while (fgets(tbuf, sizeof(rd->data[0].buf), fd) != NULL) {
            ...
            if (strncmp(tbuf, "LI", 2) == 0) {
                ...
                if (fgets(tbuf, sizeof(rd->data[0].buf), fd) == NULL) break;
...
static char *readCSVGetBuf(char *s, int len, FILE *fd, int timeout)
{
...
static int readCSVGetRow(read_t *rd, int dN, int label_N)
{
    ...
    if (N > label_N) {
        ...
        while (row < end - m) {
            if (fp_isfinite(*row)) {
                /* nothing */
            } else if (m < READ_TEXT_DEVIATE_MAX && N > label_N) {
                N--;
                m++;
            }
...
static int readCSVGetLabel(read_t *rd, int dN)
{
...
static int readCSVGetBOM(FILE *fd)
{
    char tbuf[8];
    ...
    fseek(fd, 0UL, SEEK_SET);
    if (len != 0) {
        if (memcmp(tbuf, "\x00\x00\xFE\xFF", 4) == 0 || memcmp(tbuf, "\xFF\xFE\x00\x00", 4) == 0) {
            bom = BOM_UTF_UNKNOWN;
        } else if (memcmp(tbuf, "\xFE\xFF", 2) == 0 || memcmp(tbuf, "\xFF\xFE", 2) == 0) {
            bom = BOM_UTF_UNKNOWN;
...
static int readCSVGetCN(read_t *rd, int dN, FILE *fd, fval_t *rbuf, int *rbuf_N)
{
...
static void readCloseFile(read_t *rd, int dN)
{
...
void readOpenUnified(read_t *rd, int dN, int cN, int lN, const char *file, int fmt)
{
    ...
    if (fd == NULL) {
        ...
        if (fmt == FORMAT_TEXT_STDIN || fmt == FORMAT_TEXT_CSV) {
            if (fmt != FORMAT_TEXT_STDIN) {
...
static int readSTUB(read_t *rd, int dN)
{
...
static int readCSV(read_t *rd, int dN)
{
    int rc, cN;
...
static int readFP32(read_t *rd, int dN)
{
...
static int readFP64(read_t *rd, int dN)
{
    double *fbuf = (double *) rd->data[dN].buf;
    int rc, N, cN = rd->pl->data[dN].column_N;
...
#ifdef _LEGACY
static int readLEGACY(read_t *rd, int dN)
{
...
int readDataLoad(read_t *rd)
{
    ...
    for (dN = 0; dN < PLOT_DATASET_MAX; ++dN) {
        ...
        if (fd != NULL) {
            ...
            do {
                if (rd->data[dN].format == FORMAT_TEXT_STDIN || rd->data[dN].format == FORMAT_TEXT_CSV) {
                    if (readCSV(rd, dN) != 0) {
                        ulN += 1;
                    } else {
                        break;
                ...
                if (rd->data[dN].length_N < 1 && plotDataSpaceLeft(rd->pl, dN) < 3) {
...
int readGetFreeData(read_t *rd)
{
    ...
    for (N = 0; N < PLOT_DATASET_MAX; ++N) {
        if (rd->data[N].format == FORMAT_NONE && rd->data[N].file[0] == 0) {
            dN = N;
            break;
        }
    }
...
static int configGetSym(parse_t *pa)
{
    int rc;
...
static int configUngetSym(parse_t *pa, int c)
{
    pa->unchar = c;
    return c;
}

static int configToken(read_t *rd, parse_t *pa)
{
    char *p = pa->tbuf;
    int c, n, rc = 0;
...
static void configDataMap(read_t *rd, parse_t *pa)
{
    int dN, N = 0;
...
static int configGetSubtract(read_t *rd, int pN, int fN, int axis)
{
    int N, sN = -1;
...
static int configFuzzyLabel(read_t *rd, int dN, const char *fu)
{
    char ubuf[8];
...
static void configParseFSM(read_t *rd, parse_t *pa)
{
            ...
            if (tbuf[0] == '#') {
                ...
                do {
                    ...
                    if (rc == 0) {
                        ...
                        } else if (strcmp(tbuf, "text") == 0 || strcmp(tbuf, "csv") == 0) {
                            argi[2] = FORMAT_TEXT_CSV;
                        }  else if (strcmp(tbuf, "float") == 0 || strcmp(tbuf, "fp32") == 0) {
                    ...
                    if (argi[2] == FORMAT_STUB_DATA) {
                        ...
                        failed = 0;
                        break;
                    } else if (argi[2] == FORMAT_BINARY_FP_32 || argi[2] == FORMAT_BINARY_FP_64) {
                        rc = configToken(rd, pa);
                    ...
                    if (rc == 0) {
                        ...
                        if (rd->data[dN_remap].fd == NULL) {
                            if (argi[2] == FORMAT_BINARY_FP_32 || argi[2] == FORMAT_BINARY_FP_64) {
                ...
                do {
                    ...
                    if (argi[0] >= 0 && argi[0] < PLOT_DATASET_MAX) {
                        ...
                        if (dN_remap >= 0 && rd->data[dN_remap].format != FORMAT_NONE) {
                            failed = 0;
                            rd->bind_N = dN_remap;
                        } else {
                ...
                do {
                    ...
                    if (argi[0] >= 0 && argi[0] < PLOT_AXES_MAX && argi[1] >= 0 && argi[1] < PLOT_AXES_MAX) {
                        failed = 0;
...
static char *readDirName(char *path)
{
    char *eol;
...
static void readGetUnit(char *tbuf, const char *label, int allowed)
{
    const char *text;
    int length;
...
static tuple_t readTimeDataMap(plot_t *pl, int dN, int cNX, int cNY)
{
...
static int readScaleDataMap(plot_t *pl, int dN, int cNT, int cN, subtract_t *sb)
{
    int N, gN, cMAP;
    ...
    if (gN != -1) {
        if (pl->group[gN].op_scale != 0) {
            cMAP = plotGetSubtractScale(pl, dN, cN, pl->group[gN].scale, pl->group[gN].offset);
    ...
    for (N = 0; N < READ_SUBTRACT_MAX; ++N) {
        if (sb[N].busy == SUBTRACT_SCALE) {
            cMAP = plotGetSubtractScale(pl, dN, cN, sb[N].args[0], sb[N].args[1]);
                    ...
                    if (gN != -1) {
                        ...
                        if (pl->group[gN].op_scale != 0) {
                            cMAP = plotGetSubtractScale(pl, dNT, uN.X,
                                    pl->group[gN].scale, pl->group[gN].offset);
            ...
            cMAP = plotGetSubtractMedian(pl, dN, cN, sb[N].busy, (int)sb[N].args[0]);
...
static int readCombineGetFreeAxis(plot_t *pl, int *map)
{
    int N, J, sF, aN = -1;
...
static int readCombineGetMappedAxis(plot_t *pl, int dN, int cN, int aBUSY)
{
    int N, aN = -1;
    int gN, *map;
    ...
    for (N = 0; N < PLOT_FIGURE_MAX; ++N) {
        if (pl->figure[N].busy != 0) {
            if (pl->figure[N].data_N == dN) {
                if (aBUSY == AXIS_BUSY_X && pl->figure[N].column_X == cN) {
                    aN = pl->figure[N].axis_X;
                    break;
                } else if (aBUSY == AXIS_BUSY_Y && pl->figure[N].column_Y == cN) {
            ...
            if (gN != -1) {
                ...
                if (aBUSY == AXIS_BUSY_X && gN == map[pl->figure[N].column_X]) {
                    aN = pl->figure[N].axis_X;
                    break;
                } else if (aBUSY == AXIS_BUSY_Y && gN == map[pl->figure[N].column_Y]) {
                    aN = pl->figure[N].axis_Y;
                    break;
                }
            }
...
int readDataReload(read_t *rd)
{
    ...
    for (dN = 0; dN < PLOT_DATASET_MAX; ++dN) {
        if (rd->data[dN].format != FORMAT_NONE && rd->data[dN].file[0] != 0) {
            readOpenUnified(rd, dN, rd->data[dN].column_N,
                    rd->data[dN].length_N, rd->data[dN].file, rd->data[dN].format);
