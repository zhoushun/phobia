...
static struct async_priv* async_open(int length)
{
...
static void async_close(struct async_priv *ap)
{
    free(ap->stream);
    free(ap->ophunk);
    free(ap);
...
static int async_getc(struct async_priv *ap)
{
    int rp, wp, cq;
...
static int async_putc(struct async_priv *ap, int cq)
{
    int rp, wp, wpi;
...
static int async_fgets(struct async_priv *ap, char *sbuf, int n)
{
    int rp, wp, cq, eol, nq;
...
static int async_space(struct async_priv *ap)
{
    int rp, wp, nr;
...
void serial_enumerate(struct serial_list *ls)
{
    ...
    do {
        if (QueryDosDeviceA(NULL, lpPath, uMax) != 0) break;
...
static struct serial_fd* serial_port_open(const char *devname, int baudrate, const char *mode)
{
    struct serial_fd *fd;
    char lpName[80];
    ...
    switch (baudrate) {
    case 1200:
        CommDCB.BaudRate = CBR_1200;
        break;
    case 2400:
        CommDCB.BaudRate = CBR_2400;
        break;
    case 4800:
        CommDCB.BaudRate = CBR_4800;
        break;
    case 9600:
        CommDCB.BaudRate = CBR_9600;
        break;
    case 19200:
    ...
    case 38400:
        ...
        break;
    case 57600:
        CommDCB.BaudRate = CBR_57600;
        break;
    case 115200:
        CommDCB.BaudRate = CBR_115200;
        break;
    case 128000:
        CommDCB.BaudRate = CBR_128000;
        break;
    case 256000:
        CommDCB.BaudRate = CBR_256000;
        break;
    default:
        break;
    }
...
static int serial_port_read(struct serial_fd *fd, char *s, int n)
{
    BOOL bRet;
    DWORD nBytes = 0;
...
static int serial_port_write(struct serial_fd *fd, const char *s, int n)
{
    BOOL bRet;
    ...
    while (nTotal < n) {
        ...
        bRet = WriteFile(fd->hFile, (LPCVOID)(s + nTotal), (DWORD)(n - nTotal), &nBytes, NULL);
        if (bRet == FALSE || nBytes < 1)  return SERIAL_ERROR_UNKNOWN;
...
#else /* _WINDOWS */
static void serial_port_pattern(struct serial_list *ls, const char *pat)
{
    DIR *dir;
    struct dirent *en;
    int len;
        ...
        while ((en = readdir(dir)) != NULL) {
            if (en->d_type == DT_CHR || en->d_type == DT_UNKNOWN) {
                if (memcmp(en->d_name, pat, len) == 0) {
                    ...
                    if (ls->dnum >= SERIAL_DEVICE_MAX) break;
                }
            }
        }
...
static int serial_port_write(struct serial_fd *fd, const char *s, int n)
{
    ...
    while (total < n) {
        ...
        if (rc < 1) return SERIAL_ERROR_UNKNOWN;
...
static int async_thread_rx(struct serial_fd *fd)
{
    ...
    do {
        ...
        if (SDL_AtomicGet(&ap->terminate) != 0) break;
    } while (1);
...
static int async_thread_tx(struct serial_fd *fd)
{
...
static int serial_thread_garbage(struct serial_fd *fd)
{
...
int serial_fputs(struct serial_fd *fd, const char *s)
{
    ...
    if (strlen(s) < av) {
        while (*s != 0) {
            ...
            if (rc != SERIAL_OK) return SERIAL_ERROR_UNKNOWN;
        }
