...
int ts_wait_spinup()
{
    ...
    do {
        ...
        if (pm.fsm_errno != PM_OK) break;

        if (m_fabsf(pm.s_setpoint_speed - pm.lu_wS) < pm.probe_speed_tol)
            break;

        if ( pm.lu_MODE == PM_LU_FORCED && pm.vsi_lpf_DC > pm.forced_stop_DC)
            break;
...
void ts_adjust_sensor_hall()
{
    ...
    do {
        ...
        for (N = 1; N < 7; ++N) {
            double STg = atan2(pm.hall_ST[N].Y, pm.hall_ST[N].X) * (180. / M_PI);
...
void ts_adjust_sensor_sincos()
{
    ...
    do {
        ...
        pm.s_setpoint_speed = 110.f * pm.k_EMAX / 100.f * pm.const_fb_U / pm.const_lambda;
...
static void blm_proc_DC(int A, int B, int C)
{
    m.pwm_A = A;
    m.pwm_B = B;
    m.pwm_C = C;
...
static void ts_script_speed()
{
    ...
    pm.s_setpoint_speed = 50.f * pm.k_EMAX / 100.f * pm.const_fb_U / pm.const_lambda;
    ...
    pm.s_setpoint_speed = 10.f * pm.k_EMAX / 100.f * pm.const_fb_U / pm.const_lambda;
...
static void ts_script_weakening()
{
    ...
    pm.s_setpoint_speed = 200.f * pm.k_EMAX / 100.f * pm.const_fb_U / pm.const_lambda;
...
static void ts_script_eabi(int knob_EABI)
{
    int backup_LU_ESTIMATE;
